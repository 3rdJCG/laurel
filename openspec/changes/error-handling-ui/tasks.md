...
- [x] 1.5 バージョン不整合チェックを `data:load-all` ハンドラーに追加し、未対応バージョンをスキップファイルとして扱う
...
- [x] 2.5 同種エラーの集約ロジックを実装する（同一 code のエラーは上書き）

## 3. エラーバナーの組み込み

- [x] 3.1 プロジェクト画面に保存失敗バナーを組み込む（`data:save-project` エラー時に表示）
- [x] 3.2 再試行ボタンで保存を再実行し、成功時にバナーを閉じる処理を実装する
...
- [x] 5.1 存在しないパスに保存先を変更した状態で保存操作を行い、Critical バナーが表示されることを確認する
- [x] 5.2 破損 JSON ファイルを保存先に置いた状態でアプリを起動し、Warning バナーが表示されることを確認する
- [x] 5.3 「ファイルを開く」ボタンでファイルマネージャーが開くことを確認する
