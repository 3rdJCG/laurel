...
- [x] 3.1 開発モード（`npm run dev`）でウィンドウをリサイズ・移動してから閉じ、再起動後に状態が復元されることを確認する
- [x] 3.2 `userData/window-state.json` を削除した状態で起動し、デフォルトサイズで起動することを確認する
- [x] 3.3 `window-state.json` に不正な JSON を書き込んだ状態で起動し、デフォルト値にフォールバックしアプリが正常起動することを確認する